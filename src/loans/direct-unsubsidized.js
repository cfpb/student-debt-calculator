...
unsubMax.independent = function( data ) {
  ...
  if ( data.undergrad === false ) {
    ...
    if ( data.directUnsubsidizedIndepMax > data.unsubsidizedCapIndepYearOne -
       data.directSubsidized ) {
    ...
    if ( data.directUnsubsidizedIndepMax > data.unsubsidizedCapIndepYearTwo -
      data.directSubsidized ) {
    ...
    if ( data.directUnsubsidizedIndepMax > data.unsubsidizedCapIndepYearThree -
      data.directSubsidized ) {
...
unsubMax.dependent = function( data ) {
  ...
  if ( data.undergrad === false ) {
    if ( data.directUnsubsidizedDepMax > data.unsubsidizedCapGrad -
      data.directSubsidized ) {
      data.directUnsubsidizedDepMax = data.unsubsidizedCapGrad -
                                     data.directSubsidized;
    }
  } else if ( data.program === 'aa' || data.yearInCollege === 1 ) {
    if ( data.directUnsubsidizedDepMax > data.unsubsidizedCapYearOne -
      data.directSubsidized ) {
      data.directUnsubsidizedDepMax = data.unsubsidizedCapYearOne -
                                     data.directSubsidized;
    }
  } else if ( data.yearInCollege === 2 ) {
    if ( data.directUnsubsidizedDepMax > data.unsubsidizedCapYearTwo -
      data.directSubsidized ) {
      data.directUnsubsidizedDepMax = data.unsubsidizedCapYearTwo -
                                     data.directSubsidized;
    }
  } else if ( data.yearInCollege === 3 ) {
    if ( data.directUnsubsidizedDepMax > data.unsubsidizedCapYearThree -
      data.directSubsidized ) {
      data.directUnsubsidizedDepMax = data.unsubsidizedCapYearThree -
                                     data.directSubsidized;
    }
  }
